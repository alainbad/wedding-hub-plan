...
type Creative = Tables<"suppliers">;
...
function AdminPage() {
  ...
  const setStatus = async (id: string, status: Creative["status"]) => {
    ...
    toast.success(`Creative ${STATUS_LABEL[status]?.toLowerCase()}.`);
  };

  const setPlan = async (id: string, plan: Creative["subscription_plan"]) => {
  ...
  const pending = creatives.filter((s) => s.status === "pending").length;
  const approved = creatives.filter((s) => s.status === "approved").length;

  const stats = [
    { label: "Total creatives", value: creatives.length, icon: Store },
  ...
  return (
    <div className="min-h-screen bg-secondary/20">
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
        ...
        <h2 className="font-serif text-xl font-semibold text-foreground">Creative management</h2>

        {creativesLoading ? (
          <Loader2 className="h-5 w-5 animate-spin text-muted-foreground" />
        ) : creatives.length === 0 ? (
          <Card className="p-10 text-center text-sm text-muted-foreground">No creatives registered yet.</Card>
        ) : (
              ...
              <Card key={s.id} className="flex flex-col gap-4 p-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate font-medium text-foreground">{s.company_name || "Untitled creative"}</p>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Select value={s.subscription_plan} onValueChange={(v) => setPlan(s.id, v as Creative["subscription_plan"])}>
